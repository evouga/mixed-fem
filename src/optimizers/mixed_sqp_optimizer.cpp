...
void MixedSQPOptimizer::build_lhs() {
  ...
  update_block_diagonal(Hinv_, Hs);
...
void MixedSQPOptimizer::build_rhs() {
  ...
  for (int i = 0; i < nelem_; ++i) {
    ...
    rhs_.segment<6>(6*i) = vols_(i) * h2 * diff + gs;
  }
...
void MixedSQPOptimizer::substep(bool init_guess, double& decrement) {
  ...
  // niter = pcg(la_, lhs_ , -rhs_, tmp_r_, tmp_z_, tmp_p_, tmp_Ap_, solver_);
...
void MixedSQPOptimizer::warm_start() {
  double h = config_->h;
...
VectorXd MixedSQPOptimizer::collision_force() {
