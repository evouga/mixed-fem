...
namespace mfem {
  ...
  class FixedCorotational : public MaterialModel {
  public:
    ...
    FixedCorotational(const std::shared_ptr<MaterialConfig>& config)
        : MaterialModel(config) {
      std::cout << "Creating FixedCorotational Material Model" << std::endl;
    }
