...
\danny{The above gradient and Hessian terms should be updated to the new iterate after each Newton step. If I follow your notation they look like they are staying fixed at values from time $t$ throughout?} 
...
\end{split}
\end{equation}
where each of the derivatives is evaluated at $\x^t, \s^t, \la^t$. (\danny{Again  - should be updated over Newton iterations $i$ from $t$ to next iterate $i$ - right?)} The main difficulty in these derivatives is dealing with the derivative of the rotation term. We do this by defining the rotation in terms of the SVD of the deformation gradient,
\begin{equation}
