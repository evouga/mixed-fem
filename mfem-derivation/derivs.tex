...
Or kinetic energy term for $\x = \x^k + \dx$ is
\begin{align}
E_M (\x) &= \frac{1}{2h^2} ||x^k + \dx - \tilde{x}||_M^2 \\
\end{align}
and its derivative with respect to $\dx$ is
\begin{equation}
\begin{split}
\frac{\partial E_M }{\partial \dx} &= \frac{1}{h^2}M(\dx + \x^k - \tilde{x}) \\
&=  \frac{1}{h^2}M\dx + \gx
\end{split}
\end{equation}
where $\gx = \frac{1}{h^2}M(\x^k - \tilde{x})$
...
\begin{equation}
E_\Psi (\s) \approx \tilde{E}_\Psi (\s) = \sum_{K \in \Th} \left(\Psi(\s_K^k) + \ds_K^T \gs_K + \frac{1}{2}\ds_K^T H_K^k \ds_K \right) V_K
\end{equation}
where 
$\gs_K =\left.\frac{\partial \Psi}{\partial s_K}\right|_{\s_K^k}$ and
$H_K^k=\left.\frac{\partial^2 \Psi}{\partial s_K^2}\right|_{s_K^k}$, and its derivative is simply
\begin{equation}
\frac{\partial \tilde{E}_\Psi}{\partial \ds_K} =
\sum_{K \in \Th} \left(\gs_K + H_K^k \ds_K \right) V_K
\end{equation}
